...
Accepted, process/thread/vfork and signal teardown coverage implemented;
unusual teardown follow-up
...
- `vfork()` uses `PTRACE_EVENT_VFORK` and a copied process group, so child-side
  ...
  plus `posix_spawnp()` PATH lookup and a direct `vfork()`/`execl()` child are
  covered by the static smoke test.
