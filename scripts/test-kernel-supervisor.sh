...
printf '%s\n' \
    ...
    '    return waitpid(child, NULL, 0) == child ? 0 : 6;' \
    '  }' \
    '  if (argc > 1 && strcmp(argv[1], "posix-spawnp-vfork-actions") == 0) {' \
    '#ifdef POSIX_SPAWN_USEVFORK' \
    '    char executable[PATH_MAX]; if (realpath(argv[0], executable) == NULL) return 4;' \
    '    char *slash = strrchr(executable, '\''/'\''); if (slash == NULL) return 4; *slash = '\''\0'\'';' \
    '    if (setenv("PATH", executable, 1) != 0) return 4;' \
    '    posix_spawnattr_t attributes; if (posix_spawnattr_init(&attributes) != 0) return 4;' \
    '    if (posix_spawnattr_setflags(&attributes, POSIX_SPAWN_USEVFORK) != 0) { posix_spawnattr_destroy(&attributes); return 4; }' \
    '    posix_spawn_file_actions_t actions; if (posix_spawn_file_actions_init(&actions) != 0) { posix_spawnattr_destroy(&attributes); return 4; }' \
    '    if (posix_spawn_file_actions_addopen(&actions, 12, "/dev/null", O_RDONLY, 0) != 0) { posix_spawn_file_actions_destroy(&actions); posix_spawnattr_destroy(&attributes); return 4; }' \
    '    pid_t child = -1; char *spawn_argv[] = { (char *)"static-target", (char *)"spawn-child", NULL };' \
    '    int spawn_error = posix_spawnp(&child, "static-target", &actions, &attributes, spawn_argv, environ);' \
    '    posix_spawn_file_actions_destroy(&actions); posix_spawnattr_destroy(&attributes); if (spawn_error != 0) return spawn_error;' \
    '    return waitpid(child, NULL, 0) == child ? 0 : 6;' \
    '#else' \
    '    return 77;' \
    '#endif' \
    '  }' \
    '  if (argc > 1 && strcmp(argv[1], "posix-spawn-vfork-dupclose") == 0) {' \
    '#ifdef POSIX_SPAWN_USEVFORK' \
    '    posix_spawnattr_t attributes; if (posix_spawnattr_init(&attributes) != 0) return 4;' \
    '    if (posix_spawnattr_setflags(&attributes, POSIX_SPAWN_USEVFORK) != 0) { posix_spawnattr_destroy(&attributes); return 4; }' \
    '    int fd = socket(AF_INET, SOCK_STREAM, 0); struct sockaddr_in address = {0};' \
    '    address.sin_family = AF_INET; address.sin_port = htons(47163); address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);' \
    '    if (fd < 0 || bind(fd, (struct sockaddr *)&address, sizeof(address)) < 0 || listen(fd, 4) < 0) { if (fd >= 0) close(fd); posix_spawnattr_destroy(&attributes); return 2; }' \
    '    posix_spawn_file_actions_t actions; if (posix_spawn_file_actions_init(&actions) != 0) { close(fd); posix_spawnattr_destroy(&attributes); return 4; }' \
    '    if (posix_spawn_file_actions_adddup2(&actions, fd, 9) != 0 || posix_spawn_file_actions_addclose(&actions, fd) != 0) { posix_spawn_file_actions_destroy(&actions); close(fd); posix_spawnattr_destroy(&attributes); return 4; }' \
    '    pid_t child = -1; char *spawn_argv[] = { argv[0], (char *)"spawn-child", NULL };' \
    '    int spawn_error = posix_spawn(&child, argv[0], &actions, &attributes, spawn_argv, environ);' \
    '    posix_spawn_file_actions_destroy(&actions); posix_spawnattr_destroy(&attributes); if (spawn_error != 0) { close(fd); return spawn_error; }' \
    ...
    '#endif' \
    '  }' \
    '  if (argc > 1 && strcmp(argv[1], "posix-spawnp-chdir") == 0) {' \
    '#if defined(__GLIBC__) && defined(__GLIBC_PREREQ) && __GLIBC_PREREQ(2, 29)' \
    '    char executable[PATH_MAX]; if (realpath(argv[0], executable) == NULL) return 4;' \
    '    char *slash = strrchr(executable, '\''/'\''); if (slash == NULL) return 4; *slash = '\''\0'\'';' \
    '    if (setenv("PATH", executable, 1) != 0) return 4;' \
    ...
    '#endif' \
    '  }' \
    '  if (argc > 1 && strcmp(argv[1], "posix-spawnp-closefrom") == 0) {' \
    '#if defined(__GLIBC__) && defined(__GLIBC_PREREQ) && __GLIBC_PREREQ(2, 34)' \
    '    char executable[PATH_MAX]; if (realpath(argv[0], executable) == NULL) return 4;' \
    '    char *slash = strrchr(executable, '\''/'\''); if (slash == NULL) return 4; *slash = '\''\0'\'';' \
    '    if (setenv("PATH", executable, 1) != 0) return 4;' \
    '    int fd = socket(AF_INET, SOCK_STREAM, 0); struct sockaddr_in address = {0};' \
    '    address.sin_family = AF_INET; address.sin_port = htons(47162); address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);' \
    '    if (fd < 0 || bind(fd, (struct sockaddr *)&address, sizeof(address)) < 0 || listen(fd, 4) < 0) return 2;' \
    '    posix_spawn_file_actions_t actions; if (posix_spawn_file_actions_init(&actions) != 0) return 4;' \
    '    if (posix_spawn_file_actions_addclosefrom_np(&actions, 3) != 0) { posix_spawn_file_actions_destroy(&actions); close(fd); return 77; }' \
    '    pid_t child = -1; char *spawn_argv[] = { (char *)"static-target", (char *)"spawn-child", NULL };' \
    ...
    '    return waitpid(child, NULL, 0) == child ? 0 : 6;' \
    '  }' \
    '  if (argc > 1 && strcmp(argv[1], "posix-spawnp-dup") == 0) {' \
    '    char executable[PATH_MAX]; if (realpath(argv[0], executable) == NULL) return 4;' \
    '    char *slash = strrchr(executable, '\''/'\''); if (slash == NULL) return 4; *slash = '\''\0'\'';' \
    '    if (setenv("PATH", executable, 1) != 0) return 4;' \
    '    int fd = socket(AF_INET, SOCK_STREAM, 0); struct sockaddr_in address = {0};' \
    '    address.sin_family = AF_INET; address.sin_port = htons(47161); address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);' \
    '    if (fd < 0 || bind(fd, (struct sockaddr *)&address, sizeof(address)) < 0 || listen(fd, 4) < 0) return 2;' \
    '    posix_spawn_file_actions_t actions; if (posix_spawn_file_actions_init(&actions) != 0) return 4;' \
    '    if (posix_spawn_file_actions_adddup2(&actions, fd, 9) != 0 || posix_spawn_file_actions_addclose(&actions, fd) != 0) { posix_spawn_file_actions_destroy(&actions); close(fd); return 4; }' \
    '    pid_t child = -1; char *spawn_argv[] = { (char *)"static-target", (char *)"spawn-child", NULL };' \
    '    int spawn_error = posix_spawnp(&child, "static-target", &actions, NULL, spawn_argv, environ);' \
    '    posix_spawn_file_actions_destroy(&actions); if (spawn_error != 0) { close(fd); return spawn_error; }' \
    '    int result = waitpid(child, NULL, 0) == child ? 0 : 6; close(fd); return result;' \
...
start_control "$tmp_dir/posix-spawnp-vfork-actions.sock" "$tmp_dir/posix-spawnp-vfork-actions.log"
set +e
WSL_WIN_RELAY_CONTROL="$tmp_dir/posix-spawnp-vfork-actions.sock" "$repo_dir/scripts/wsl-win-relay-run" --kernel "$tmp_dir/static-target" posix-spawnp-vfork-actions
posix_spawnp_vfork_actions_status=$?
set -e
if [ "$posix_spawnp_vfork_actions_status" -ne 0 ] && [ "$posix_spawnp_vfork_actions_status" -ne 77 ]; then
    echo "posix_spawnp(POSIX_SPAWN_USEVFORK + actions) target failed with status $posix_spawnp_vfork_actions_status" >&2
    exit 1
fi
if [ "$posix_spawnp_vfork_actions_status" -eq 0 ]; then
    grep -q 'RESERVE .* tcp4 47147' "$tmp_dir/posix-spawnp-vfork-actions.log"
    grep -q '^COMMIT ' "$tmp_dir/posix-spawnp-vfork-actions.log"
    grep -Eq '^(CLOSE|RELEASE) ' "$tmp_dir/posix-spawnp-vfork-actions.log"
fi
stop_control

start_control "$tmp_dir/posix-spawn-vfork-dupclose.sock" "$tmp_dir/posix-spawn-vfork-dupclose.log"
set +e
WSL_WIN_RELAY_CONTROL="$tmp_dir/posix-spawn-vfork-dupclose.sock" "$repo_dir/scripts/wsl-win-relay-run" --kernel "$tmp_dir/static-target" posix-spawn-vfork-dupclose
posix_spawn_vfork_dupclose_status=$?
set -e
if [ "$posix_spawn_vfork_dupclose_status" -ne 0 ] && [ "$posix_spawn_vfork_dupclose_status" -ne 77 ]; then
    echo "posix_spawn(POSIX_SPAWN_USEVFORK + dup2/close) target failed with status $posix_spawn_vfork_dupclose_status" >&2
    exit 1
fi
if [ "$posix_spawn_vfork_dupclose_status" -eq 0 ]; then
    grep -q 'RESERVE .* tcp4 47163' "$tmp_dir/posix-spawn-vfork-dupclose.log"
    grep -q '^COMMIT ' "$tmp_dir/posix-spawn-vfork-dupclose.log"
    grep -Eq '^(CLOSE|RELEASE) ' "$tmp_dir/posix-spawn-vfork-dupclose.log"
fi
stop_control
...
start_control "$tmp_dir/posix-spawnp-dup.sock" "$tmp_dir/posix-spawnp-dup.log"
WSL_WIN_RELAY_CONTROL="$tmp_dir/posix-spawnp-dup.sock" "$repo_dir/scripts/wsl-win-relay-run" --kernel "$tmp_dir/static-target" posix-spawnp-dup
grep -q 'RESERVE .* tcp4 47147' "$tmp_dir/posix-spawnp-dup.log"
grep -q 'RESERVE .* tcp4 47161' "$tmp_dir/posix-spawnp-dup.log"
grep -q '^ADOPT ' "$tmp_dir/posix-spawnp-dup.log"
grep -Eq '^(CLOSE|RELEASE) ' "$tmp_dir/posix-spawnp-dup.log"
stop_control
...
start_control "$tmp_dir/posix-spawnp-chdir.sock" "$tmp_dir/posix-spawnp-chdir.log"
set +e
WSL_WIN_RELAY_CONTROL="$tmp_dir/posix-spawnp-chdir.sock" "$repo_dir/scripts/wsl-win-relay-run" --kernel "$tmp_dir/static-target" posix-spawnp-chdir
posix_spawnp_chdir_status=$?
set -e
if [ "$posix_spawnp_chdir_status" -ne 0 ] && [ "$posix_spawnp_chdir_status" -ne 77 ]; then
    echo "posix_spawnp chdir target failed with status $posix_spawnp_chdir_status" >&2
    exit 1
fi
if [ "$posix_spawnp_chdir_status" -eq 0 ]; then
    grep -q 'RESERVE .* tcp4 47147' "$tmp_dir/posix-spawnp-chdir.log"
    grep -q '^COMMIT ' "$tmp_dir/posix-spawnp-chdir.log"
    grep -Eq '^(CLOSE|RELEASE) ' "$tmp_dir/posix-spawnp-chdir.log"
fi
stop_control
...
start_control "$tmp_dir/posix-spawnp-closefrom.sock" "$tmp_dir/posix-spawnp-closefrom.log"
set +e
WSL_WIN_RELAY_CONTROL="$tmp_dir/posix-spawnp-closefrom.sock" "$repo_dir/scripts/wsl-win-relay-run" --kernel "$tmp_dir/static-target" posix-spawnp-closefrom
posix_spawnp_closefrom_status=$?
set -e
if [ "$posix_spawnp_closefrom_status" -ne 0 ] && [ "$posix_spawnp_closefrom_status" -ne 77 ]; then
    echo "posix_spawnp closefrom target failed with status $posix_spawnp_closefrom_status" >&2
    exit 1
fi
if [ "$posix_spawnp_closefrom_status" -eq 0 ]; then
    grep -q 'RESERVE .* tcp4 47162' "$tmp_dir/posix-spawnp-closefrom.log"
    grep -q 'RESERVE .* tcp4 47147' "$tmp_dir/posix-spawnp-closefrom.log"
    test "$(grep -Ec '^ADOPT ' "$tmp_dir/posix-spawnp-closefrom.log")" -ge 1
    test "$(grep -Ec '^(CLOSE|RELEASE) ' "$tmp_dir/posix-spawnp-closefrom.log")" -ge 2
fi
stop_control
