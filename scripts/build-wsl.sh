...
GOTOOLCHAIN=local GOOS=windows GOARCH=amd64 go build -o "$repo_dir/bin/wsl-win-broker.exe" "$repo_dir/cmd/win-broker"
GOTOOLCHAIN=local GOOS=windows GOARCH=amd64 go build -o "$repo_dir/bin/wsl-win-connector.exe" "$repo_dir/cmd/win-connector"
